...
export function VantaWorld({ agents }: VantaWorldProps): JSX.Element {
  ...
  if (!mounted) {
    return (
      <div className="relative h-full w-full overflow-hidden rounded-2xl border border-ink-800 bg-[#cfe6ff]">
        <div className="absolute inset-0 flex items-center justify-center font-mono text-xs uppercase tracking-[0.22em] text-ink-700">
          mounting world…
        </div>
      </div>
    );
  ...
  return (
    <div className="relative h-full w-full overflow-hidden rounded-2xl border border-ink-800 bg-[#bcd9f5]">
      <Canvas
        shadows
...
function Scene({ buckets }: SceneProps): JSX.Element {
  ...
  return (
    <>
      ...
      {/* Floating banner rings above each kingdom CAPITAL */}
      {buckets.opus ? (
        <FloatingRing
          {...hexToWorld(0, 6)}
          y={2.4}
...
function SkyAndLights(): JSX.Element {
  return (
    <>
...
function FloatingRing({ x, y, z, color }: FloatingRingProps): JSX.Element {
  const ref = useRef<THREE.Mesh>(null);
  useFrame((state) => {
    if (!ref.current) return;
    const t = state.clock.elapsedTime;
    ...
    ref.current.position.y = y + Math.sin(t * 1.2 + x) * 0.18;
  });
  return (
