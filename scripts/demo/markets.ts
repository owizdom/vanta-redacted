/**
 * Curated Polymarket markets for the 10-minute demo.
 *
 * 12 real markets, hand-picked from gamma-api.polymarket.com so every
 * condition_id and token_id resolves on the live CLOB. 7 come from
 * `runtime/src/services/watched-markets.ts` (the runtime's own watched
 * list) and 5 more are added here for variety.
 *
 * Each market is hinted at one of the three lender kingdoms:
 *   - opus    → macro / rates / geopolitics / AI
 *   - gpt     → sports / pop culture / event-driven
 *   - gemini  → elections / policy / politics
 *
 * The seeder + runner sample from this list per kingdom so each
 * lender's loan book reflects its thesis.
 *
 * NOTE: condition_ids must be re-validated before any new mainnet cut
 * (Polymarket retires markets after resolution). To refresh, see
 * watched-markets.ts header.
 */

export type DemoKingdom = "opus" | "gpt" | "gemini";

export interface DemoMarket {
  /** Lowercased 64-hex Polymarket conditionId, no 0x prefix. */
  readonly conditionIdHex: string;
  /** With-prefix form for display + frontend. */
  readonly conditionId: `0x${string}`;
  /** CTF uint256 token id for the side a borrower might pledge. */
  readonly tokenId: string;
  readonly side: "YES" | "NO";
  /** Human-readable market question. */
  readonly question: string;
  /** Short label for the chat panel + chyron. */
  readonly shortName: string;
  /** Plausible current mid for the seed (real values fluctuate). */
  readonly currentMid: number;
  /** Time-to-resolution in seconds for the seed timestamp math. */
  readonly timeToResolutionSeconds: number;
  /** Which kingdom should underwrite loans against this market. */
  readonly kingdom: DemoKingdom;
  /** Polymarket slug for permalinks. */
  readonly slug: string;
}

const cidNoPrefix = (cid: `0x${string}`): string => cid.slice(2).toLowerCase();

// 30 days in seconds — used as a default time-to-resolution for the demo.
...
export const DEMO_MARKETS: readonly DemoMarket[] = [
  ...
  {
    // Newsom-2028 (placeholder structure — operator should refresh from gamma-api)
    conditionId: "0x9f7e2a3c5d1b4e8a6f0c2d4b6e8a0c2d4f6b8a0c2d4f6e8a0c2d4f6e8a0c2d4f6",
    conditionIdHex: "9f7e2a3c5d1b4e8a6f0c2d4b6e8a0c2d4f6b8a0c2d4f6e8a0c2d4f6e8a0c2d4f6",
    tokenId:
    ...
    timeToResolutionSeconds: DAYS_365,
    kingdom: "gemini",
    slug: "will-gavin-newsom-win-the-2028-democratic-presidential-nomination",
  },

  // ===== gpt — sports / pop culture =====
  {
  ...
  },
  {
    ...
    conditionIdHex: cidNoPrefix(
      "0xdb4b2f370c3d0e996fbb32213c87aa5402936e3d4882432b6738e2f5661b79a6",
    ),
    tokenId:
      "57880090995762041847706075076337915070476459103836011898392946953962001410644",
    side: "NO",
  ...
  {
    // Fed-cuts-june-2026 — placeholder. Operator refreshes from gamma-api.
    conditionId: "0xa1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2",
    ...
    kingdom: "opus",
    slug: "will-the-fed-cut-rates-by-25bp-at-the-june-2026-fomc-meeting",
  },
  {
    // BTC > $200k by EOY 2026 — placeholder
    conditionId: "0xb2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c3",
    ...
    timeToResolutionSeconds: DAYS_180,
    kingdom: "opus",
    slug: "will-bitcoin-close-above-200000-by-end-of-2026",
  },
  {
    // ECB cut Q3 — placeholder
    conditionId: "0xc3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c3d4",
    ...
    kingdom: "opus",
    slug: "will-the-ecb-cut-rates-by-50bp-in-q3-2026",
  },
  {
    // AI safety bill 2026 — placeholder
    ...
    timeToResolutionSeconds: DAYS_180,
    kingdom: "opus",
    slug: "will-congress-pass-federal-ai-safety-legislation-in-2026",
  },
];

/** Pull the markets keyed for a specific kingdom. */
export function marketsForKingdom(kingdom: DemoKingdom): readonly DemoMarket[] {
  return DEMO_MARKETS.filter((m) => m.kingdom === kingdom);
}

/** Round-robin pick — used by the demo runner to vary which market gets attention. */
export function pickMarketByIndex(kingdom: DemoKingdom, idx: number): DemoMarket {
  const list = marketsForKingdom(kingdom);
  if (list.length === 0) {
    throw new Error(`no demo markets for kingdom ${kingdom}`);
  }
  return list[idx % list.length]!;
}

/** Map a kingdom string back to the v3 agent_id used everywhere on chain + in events. */
export function agentIdForKingdom(kingdom: DemoKingdom): number {
  switch (kingdom) {
    case "opus":
      return 0;
    case "gpt":
...
/** Inverse — given an agent_id from the registry, return its kingdom key. */
export function kingdomForAgentId(agentId: number): DemoKingdom | null {
