...
VANTA is the autonomous risk engine that sits where Gondor's curator sits. Same product, different brain. The brain is not one model but a council of three reasoning personas rotating on a 45-second tick inside the same enclave: \textbf{vanta-opus} on Anthropic Claude Sonnet 4.6 (conservative scholar, long horizons, tight haircuts), \textbf{vanta-gpt} on OpenAI GPT-5 (high-frequency underwriter, short horizons, tactical), and \textbf{vanta-gemini} on Google Gemini 2.5 Pro (politics-savvy, policy-shock loans). No single model dictates a loan; every haircut is the synthesis of three independent perspectives. Above the council sits a sovereign agent running three continuous reasoning loops --- credit, model, operational (\S\ref{sec:loops}) --- on top of a deterministic floor of formula-priced haircuts (\S\ref{sec:haircut}) and contract-enforced onboarding gates (\S\ref{sec:onboard}). The floor sets what the contract will accept; the reasoning sets what the agent proposes within that envelope --- per-market caps inside the \$25k probation ceiling, calibration-drift flags to lender governance, per-loan health monitoring, cluster-correlation alerts above the explicit tag system. Every reasoning step is a typed signed event with full input provenance; the chain-of-thought is auditable and bounded, not opaque. The agent runs inside an EigenCompute Intel TDX enclave; its Ed25519 signing key is generated in enclave RAM at boot and never leaves. The on-chain contracts (\texttt{LpVault}, \texttt{LoanBook}, \texttt{VantaVault}) are owned by an address only the running enclave image can derive. The team's role shifts from ongoing fund manager to one-time image publisher; trust shrinks from operational to one-time auditor review. Beyond lending, VANTA also operates as a Service Agent under the X402 protocol (\S\ref{sec:commerce}): it self-funds its own model inference through the Eigen AI Gateway and, in turn, sells signed quotes and marks to other agents in USDC on Base mainnet, with both flows recorded on the same canonical log as the loans themselves.
...
All five derivations happen inside the enclave; none of the derived keys leave it. The current deployments are on mainnet: \texttt{LpVault} at \texttt{0xe2f93c448d9fc51155e2e06479b3b1e86f8ae45b} on Base, \texttt{LoanBook} at \texttt{0x7ed4e98d460bbd7e43854cd93fd96d8e11b71954} on Base, \texttt{VantaVault} at \texttt{0xe2f93c448d9fc51155e2e06479b3b1e86f8ae45b} on Polygon. All three are owned by the in-enclave admin address \texttt{0x2F86357658C5CF8A5D2221b9935412C880476B14}; the X402 service-agent treasury \texttt{0x667E3116C7eA909f97dD35167c2927BfAf744B7F} is HKDF-derived under a separate info string so a metering breach cannot mint loans. EigenCompute App ID on \texttt{mainnet-alpha} is \texttt{0x95F2AB29fAa9A4C834B06B0514428d63C6e0E80d}, anchored on Ethereum mainnet.
...
% =========================================================
\section{Agent Commerce: A Paid Service Surface}
\label{sec:commerce}

VANTA is two economic roles at once. The first role is borrower-facing: the agent originates loans and earns fees on principal. The second role is provider-facing: the agent itself sells signed work to other agents and to humans, settling in USDC on Base mainnet. Both flows credit the same in-enclave-derived treasury wallet, and both surface as typed signed events on the same canonical log.

\textbf{The metered surface.} The runtime exposes two HTTP endpoints under the Coinbase X402 protocol~\cite{coinbase-x402} (\texttt{exact} scheme, EIP-3009 USDC~\cite{eip3009}):

\begin{itemize}[leftmargin=*,itemsep=2pt,topsep=2pt]
  \item \texttt{POST /bridge/wizard/quote} --- a TEE-signed haircut quote for a Polymarket position; \$0.05 USDC; the response body carries the issuer pubkey, the formula inputs, the recommended haircut, the maximum loan, and an Ed25519 signature over the canonical-JSON inner.
  \item \texttt{GET /mark/:market\_id} --- a TEE-signed 30-minute TWAP read for a market; \$0.001 USDC.
\end{itemize}

A discovery document at \texttt{GET /.well-known/x402} advertises the prices, the receiver wallet, the asset (USDC on Base mainnet, EIP-712 domain pinned to \texttt{(name="USDC", version="2", chainId=8453)}), and the issuer pubkey, so an external agent can resolve everything it needs to verify a quote without reading source.

\textbf{Settlement flow.} A caller without an \texttt{X-PAYMENT} header receives \texttt{402 Payment Required} plus a JSON challenge: price, receiver wallet, asset, 60-second validity. The caller signs a \texttt{TransferWithAuthorization} typed-data payload to the treasury wallet, base64-encodes the envelope, and retries with \texttt{X-PAYMENT: <envelope>}. The runtime recovers the signer against the USDC v2 EIP-712 domain, verifies the recovered address equals the \texttt{from} field, checks the nonce is not already used via \texttt{authorizationState(authorizer, nonce)}, then calls USDC's \texttt{transferWithAuthorization} directly. Settlement is one transaction; the runtime relays gas in ETH out of the same treasury. On success, \texttt{X-PAYMENT-RESPONSE} returns the on-chain tx hash and block number, and the runtime appends a typed \texttt{treasury.inflow} event:

\begin{lstlisting}
TreasuryInflow {
  txHash:      bytes32,
  chainId:     uint32,        // 8453 (Base mainnet)
  asset:       address,       // USDC
  amount:      uint256,       // raw 6-dec value
  fromAddr:    address,       // payer
  toAddr:      address,       // treasury wallet
  blockNumber: uint64,
  timestamp:   uint64,
  signature:   bytes65        // Ed25519 from in-enclave key
}
...
\textbf{Two trust properties this composition adds.} First, the receiver of every X402 inflow is the HKDF-derived treasury wallet whose private key never leaves the enclave; it is reproducible only inside the same image whose digest is pinned in the on-chain attestation anchor. There is no operator-controlled hot wallet sitting between the buyer and the agent. Second, the issuer pubkey on every quote and mark is the same Ed25519 key that signs \texttt{loan.origination} (\S\ref{sec:settlement}). A buyer of a quote can publish the quote and any third party can verify it came from the same verified build that the contract trusts to mint loans. Reasoning, signatures, and economic flows resolve to one root of trust.

\textbf{Why this matters for the autonomy claim.} \S\ref{sec:verify} argues that the team's role shrinks from ongoing fund manager to one-time image publisher. Agent commerce closes the corresponding loop on the income side: every inflow is a signed event auditable on the same canonical log as outflows, so the agent's full economic life --- cognition spend (LLM inference paid through the Eigen AI Gateway authenticated by the KMS-attested JWT), service inflows (X402 metered routes), loan outflows (\texttt{LoanBook.originate}), settlements (\texttt{markRepaid} / \texttt{markLiquidated}), gas refills --- is one append-only stream. A lender doing the audit of \S\ref{sec:verify} reads the same log to verify the agent received what it claims to have received and spent what it claims to have spent.

\textbf{Origination/key separation.} The treasury wallet \texttt{0x667E3116C7eA909f97dD35167c2927BfAf744B7F} and the origination admin \texttt{0x2F86357658C5CF8A5D2221b9935412C880476B14} are distinct HKDF derivations under different info strings (Invariant~1 spec, runtime \texttt{bootstrap.ts}). Distinct keys for distinct concerns: a metering breach on the X402 surface cannot mint loans, and a compromise of the origination key cannot be paid for through the X402 inflow path. Both keys remain enclave-bound; the separation is a defense-in-depth against single-key blast radius, not a substitute for the TEE.

\textbf{What this is not.} The X402 surface is not a primary revenue source for v1; volumes are early. It is the architectural commitment that VANTA's economic in-flows and out-flows are both verifiable, and the substrate for forward roadmap items (\S\ref{sec:roadmap}: pledge-bundling endpoint, on-demand inference, signed dispute analyses) that extend the agent-commerce surface area without changing the lending core.
...
\begin{verbatim}
  ...
  |                /bridge/wizard/quote  /mark/:market_id  (X402-metered) |
  |                /.well-known/x402  (discovery: prices + receiver)      |
  +----+----------------------------+--------+----------------------------+
       | signed events              | on-chain txs
       v                            v
  +-- Off-chain log -----------+  +-- On-chain ----------------------+
  | Walrus + IPFS (full log)   |  | Base mainnet (chain 8453):       |
  | Weekly tip anchor on Base  |  |   LpVault    0xe2f93c...ae45b    |
  | Content-addressed events   |  |   LoanBook   0x7ed4e9...1954     |
  +----------------------------+  |   USDC       0x833589...2913     |
  ...
  +-----------------------+      |   against LoanBook on Base mainnet.  |
                                 +--------------------------------------+
\end{verbatim}
\end{minipage}
...
\textbf{Now (shipped).} \texttt{LpVault} (\texttt{0xe2f93c...ae45b}) and \texttt{LoanBook} (\texttt{0x7ed4e9...1954}) on Base mainnet, \texttt{VantaVault} (\texttt{0xe2f93c...ae45b}) on Polygon mainnet --- all owned by the in-enclave admin address \texttt{0x2F86357658C5CF8A5D2221b9935412C880476B14}. EigenCompute App ID \texttt{0x95F2AB29fAa9A4C834B06B0514428d63C6e0E80d} on \texttt{mainnet-alpha} (anchored on Ethereum mainnet) runs the haircut formula, three reasoning loops (\S\ref{sec:loops}), and the X402 service-agent surface (\S\ref{sec:commerce}, two metered endpoints settling to the HKDF-derived treasury \texttt{0x667E3116C7eA909f97dD35167c2927BfAf744B7F} on Base). Eleven TypeScript packages (tee, events, treasury, constitution, venue-poly, mark, pledge, haircut, origination, verify, runtime); \texttt{vanta-verify} walks the chain to genesis. The live frontend at \texttt{vanta-app.vercel.app} reads the verified attestation block via \texttt{GET /api/tee} and shows it on every page; the X402 discovery doc is live at \texttt{/.well-known/x402}.

\textbf{Next --- onboarding gates and image audit.} Implement the seven-gate cycle of \S\ref{sec:onboard}, with the throughput cap and probation envelope enforced contract-side. Publish the image build with reproducible-build instructions, request audit from one of \{Trail of Bits, OpenZeppelin, Code4rena\}. Auditor signs the image measurement; signature is committed in the contract. The first public depositor checks the auditor's signature, not the image themselves.

\textbf{Then --- attestation-liveness exit and governance multisig.} Ship Invariant~5 to contract: 14-day silence triggers Withdraw-Only mode; permissionless withdrawal at TWAP-marked NAV. Deploy 4-of-7 lender-quorum multisig for emergency pause and parameter calibration commits. Calibration v0 published with public 12-month replay dataset and reproducible calibration code.

\textbf{Then --- TVL ramp.} v1 mainnet currently runs against a soft cap with no public deposits invited; the bond-and-flag system, the probation envelope, and the lender-quorum multisig must all be live before the first public-depositor cohort. Ramp follows the absence of T1--T9 events, not a calendar.

\textbf{Then --- per-agent on-chain isolation.} Source for \texttt{AgentRegistry}, \texttt{AgentFactory}, per-VANTA \texttt{AgentPoolVault}, \texttt{PositionBook}, and \texttt{OperationalCap} already lives in \texttt{contracts/src/}. Deploy \texttt{AgentRegistry} on Base mainnet, then \texttt{AgentFactory.deploy()} once per persona to atomically create that persona's per-agent vault + position book + operational cap and register on chain. Runtime stub in \texttt{runtime/src/services/agent-registry-reader.ts} already includes a \texttt{createViemRegistryReader}; the \texttt{infra\_mode} field on \texttt{/api/agents} flips from \texttt{shared-pool-v1} to \texttt{per-agent-v2} once the registry exists. v1 ships shared-pool because deploying four contracts before having a single real loan would have been theatre.

\textbf{Then --- multi-position borrower UX.} Ship the batched-origination event schema; deploy a position-portfolio view served from the same signed event log the verifier reads; extend the credit loop (\S\ref{sec:loops}) with an exit planner that schedules correlated settlements in a single transaction. A holder of ten Polymarket positions originates ten loans in one user action and settles correlated positions in batches as their cluster events resolve. VantaVault tracks position-level escrow already, so this is a user-facing surface plus an agent-side scheduler over state the credit loop already maintains, not a new contract subsystem.

\textbf{Then --- additional X402 service surfaces.} Pledge-bundling endpoint, on-demand model-of-the-day inference, signed dispute analyses; each one extends VANTA's agent-commerce surface area (\S\ref{sec:commerce}) without changing the lending core.

\textbf{Later.} Per-agent TEE signing keys (three separate EigenCompute apps, one per persona, for cryptographic isolation between vanta-opus / vanta-gpt / vanta-gemini). Tranches. Cross-margining (Gondor V1's diversified-collateral model, but with formula-derived correlation haircuts). Whitelisted Hyperliquid HIP-4 markets~\cite{hyperliquid-hip4} when their resolution criteria stabilize. Kalshi event-contract integration is gated on a separate FCM-compliance review and is not in current scope.
...
\bibitem{coinbase-x402} Coinbase. \emph{x402: an open standard for HTTP-native payments.} 2025. \url{https://github.com/coinbase/x402}.

\bibitem{eip3009} P. Choi and E. Bertanian. \emph{ERC-3009: Transfer with Authorization.} Ethereum Improvement Proposals, 2019. \url{https://eips.ethereum.org/EIPS/eip-3009}.
